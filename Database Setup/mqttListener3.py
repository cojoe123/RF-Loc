...
def on_connect(client, userdata, flags, rc):
    ...
    #sub to detectors
    client.subscribe("dt2db/d3")
    
    #sub to detector crypto
    client.subscribe("dt2db/d3crypto")
...
def on_message(client, userdata, message):
    ...
    if message.topic=="dt2db/d3" and len(str(message.payload.decode("utf-8"))) > 10:    
        #SQL Code Here
        s1=str(message.payload.decode("utf-8"))
        m1,s1,t1=s1.split(",")
        try: 
            cur.execute("INSERT INTO rf_loc_proberequestframe (mac_address, time_stamp, signal_strength, detector) VALUES (%s, %s, %s, %s)", (m1,t1,s1, "detector-3"))
        except psycopg2.Error as e:
            pass
        status=cur.rowcount
        #if insertion worked, publish "Good!" to corresponding topic, else publish "Bad!"
        if status==1:
            client.publish("db2dt/d3", "Good!")
            conn.commit()
        else:
            client.publish("db2dt/d3", "Bad")
        
#<----------------------------------------------------Handling Crypto--------------------------------------------->
#                                            (Travis or Joe Please try to do this)

    elif message.topic=="dt2db/d3crypto":            
        pass
